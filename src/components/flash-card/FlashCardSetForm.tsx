...
const FlashcardSetForm: React.FC<FlashcardFormProps> = ({
  flashcardList,
  isAdd,
  onSubmit,
}) => {
  const [flashcardSet, setFlashcardSet] =
    useState<IFlashCardList>(flashcardList);
  ...
  const handleAddCard = () => {
    ...
    setFlashcardSet((prevFlashcardSet) => ({
      __v: prevFlashcardSet.__v,
      _id: prevFlashcardSet._id,
      name: prevFlashcardSet.name,
      userId: prevFlashcardSet.userId,
          ...
          <Grid container spacing={2} key={card._id} sx={{ mt: 1 }}>
            ...
            >
              <Typography variant="h6">
                {" "}
                {flashcardSet.flashcards.length - index}
              </Typography>
            </Grid>
            <Grid item xs={5}>
              <TextField
